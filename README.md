...
The grammar files are used by the [`apglib`](https://github.com/ldthomas/apg-js2) library to generate the `grammarObject.js`.
...
This tool is deployed using `npm publish` command. 
The version should be updated in the `package.json` file.
Once the version is updated, you should create a new release on GitHub with the new version number, and update the `Script tag` section of this README with the last version number.
